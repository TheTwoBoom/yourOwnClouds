...
if (place_meeting(x, y, ServerCase)) { 
    y = ServerCase.y + 314;
    sprite_index = FanTopSprite;

    // Falls der Fan bereits in einem Slot ist, diesen speichern
    var current_slot = -1;
    for (var i = 1; i <= 4; i++) {
        if (global.mainboard.fan_slot[i] == id) {
            current_slot = i;
            break;
        }
    }
    ...
    if (current_slot == -1) {
        for (var i = 1; i <= 4; i++) {
            if (global.mainboard.fan_slot[i] == "empty") {
                current_slot = i;
                global.mainboard.fan_slot[i] = id; // Belege den Slot
                break;
            }
        }
    }
    ...
    if (current_slot != -1) {
        x = ServerCase.x + (current_slot - 1) * 72 + 13;
    }
} else {
    ...
    for (var i = 1; i <= 4; i++) {
        if (global.mainboard.fan_slot[i] == id) {
            global.mainboard.fan_slot[i] = "empty"; // Slot leeren
            removed = true;
            break;
        }
    }
